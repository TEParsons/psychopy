...
class Theme:
    ...
    def loadPluginThemes():
        ...
        # iterate through relevant entry points for app color schemes
        for ep in getEntryPointGroup("psychopy.app.themes.colors.app"):
            # try to load (import) each one
            try:
                ep.load()
            except Exception as err:
                logging.error(
                    f"Failed to load {ep.name} from {ep.value}: {err}"
                )
