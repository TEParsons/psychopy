...
class FlowPanel(wx.Panel):
    ...
    class FlowCanvas(wx.ScrolledWindow):
        class RoutineButton(wx.Button):
            ...
            def _applyAppTheme(self):
                self.SetBackgroundColour(ThemeMixin.appColors['fl_routine_bg_slip'])
                self.SetForegroundColour(ThemeMixin.appColors['fl_flowline_fg'])

        class FlowGap(wx.Button):
            def __init__(self, parent):
                wx.Button.__init__(self, parent,
                                   label="",
                                   size=(50, 50),
            ...
            def onClick(self, event=None):
                pos = self.parent.sizer.GetItemPosition(self)
                if self.parent.state == "Routine":
                    self.parent.addRoutine(pos[1])
                elif self.parent.state == "LoopEnd":
                    self.parent.addLoopEnd(pos[1])
                elif self.parent.state == "LoopStart":
                    self.parent.addLoopStart(pos[1])

            def hoverOn(self, event=None):
                if self.parent.state in ['Routine', 'LoopStart', 'LoopEnd']:
                    self.SetLabel(chr(int("2022", 16)))
                    self.Update()
                else:
                    self.SetLabel("")
                    self.Update()

            def hoverOff(self, event=None):
                self.SetLabel("")
                self.Update()
        ...
        class LoopCtrl(wx.Button):
            ...
            def _applyAppTheme(self):
                self.SetBackgroundColour(ThemeMixin.appColors['fl_flowline_bg'])
                self.SetForegroundColour(ThemeMixin.appColors['fl_flowline_fg'])
        ...
        def __init__(self, parent, buttons):
            ...
            self.sizer.Add(self.FlowGap(self), pos=(1, 1))
            self.SetSizer(self.sizer)

            self.state = False
            self.openLoop = False

        def addRoutine(self, col):
            newRt = self.RoutineButton(self, "")
            # Shift everything else up
            if col+1 < self.sizer.GetCols():
                for i in range(col+1, self.sizer.GetCols()).__reversed__():
                    obj = self.sizer.FindItemAtPosition((1, i))
                    if obj:
                        self.sizer.SetItemPosition(obj.Window, (1, i+2))
            # Insert routine and a gap
            self.sizer.Add(newRt, pos=(1, col+1))
            self.sizer.Add(self.FlowGap(self), pos=(1, col+2))
        ...
        def addLoopEnd(self, col):
            newLoopEnd = self.LoopEnd(self)
            # Shift everything else up
            if col + 1 < self.sizer.GetCols():
                for i in range(col + 1, self.sizer.GetCols()).__reversed__():
                    obj = self.sizer.FindItemAtPosition((1, i))
                    if obj:
                        self.sizer.SetItemPosition(obj.Window, (1, i + 2))
            # Insert loop end
            self.sizer.Add(newLoopEnd, pos=(1, col + 1))
            self.sizer.Add(self.FlowGap(self), pos=(1, col + 2))
            self.openLoop = newLoopEnd
            ...
            if col + 1 < self.sizer.GetCols():
                for i in range(col + 1, self.sizer.GetCols()).__reversed__():
                    ...
                    if obj:
                        self.sizer.SetItemPosition(obj.Window, (1, i + 2))
            # Insert loop end
            self.sizer.Add(newLoopStart, pos=(1, col + 1))
            self.sizer.Add(self.FlowGap(self), pos=(1, col + 2))
        ...
        def state(self, value):
            if value in ['Routine']:
                self._state = value
                self.buttons['insertRoutine'].SetBackgroundColour(cs['txtbutton_bg_hover'])
                self.buttons['insertRoutine'].SetForegroundColour(cs['txtbutton_fg_hover'])
                self.buttons['insertRoutine'].SetLabel("CANCEL Insert")
                self.buttons['insertRoutine'].Update()
            elif value in ['LoopStart', 'LoopEnd']:
                self._state = value
                self.buttons['insertLoop'].SetBackgroundColour(cs['txtbutton_bg_hover'])
                self.buttons['insertLoop'].SetForegroundColour(cs['txtbutton_fg_hover'])
                self.buttons['insertLoop'].SetLabel("CANCEL Insert")
                self.buttons['insertLoop'].Update()
            else:
                self._state = False
                self.buttons['insertRoutine'].SetBackgroundColour(cs['frame_bg'])
                self.buttons['insertRoutine'].SetForegroundColour(cs['text'])
                self.buttons['insertRoutine'].SetLabel("Insert Routine")
                self.buttons['insertRoutine'].Update()
                self.buttons['insertLoop'].SetBackgroundColour(cs['frame_bg'])
                self.buttons['insertLoop'].SetForegroundColour(cs['text'])
                self.buttons['insertLoop'].SetLabel("Insert Loop")
                self.buttons['insertLoop'].Update()
    ...
    def __init__(self, frame, id=-1):
        ...
        self.dpi = self.app.dpi
        self.prefs = self.app.prefs
        panelHeight = 3 * self.dpi
        wx.Panel.__init__(self, frame, id, (0, 0),
                          size=wx.Size(8 * self.dpi, panelHeight),
                          style=wx.BORDER_NONE)
        # Setup sizer
        ...
        self.sizer.Add(self.btnSizer, border=6, flag=wx.ALL)
        self.buttons = {}
        self.buttons['insertRoutine'] = wx.ToggleButton(self, label="Insert Routine",
                                                        size=(136, self.dpi/2), style=wx.NO_BORDER)
        self.buttons['insertRoutine'].Bind(wx.EVT_TOGGLEBUTTON, self.insertRoutine)
        self.buttons['insertLoop'] = wx.ToggleButton(self, label="Insert Loop",
                                                     size=(136, self.dpi/2), style=wx.NO_BORDER)
        self.buttons['insertLoop'].Bind(wx.EVT_TOGGLEBUTTON, self.insertLoop)
        for key, btn in self.buttons.items():
            self.btnSizer.Add(btn, border=3, flag=wx.ALL, proportion=1)
        # Add canvas
        self.canvas = self.FlowCanvas(self, self.buttons)
    ...
    def insertRoutine(self, event=None):
        if self.canvas.state == False:
            self.canvas.state = "Routine"
        else:
            self.canvas.state = False

    def insertLoop(self, event=None):
        if self.canvas.state == False:
            self.canvas.state = "LoopEnd"
        else:
            self.canvas.state = False
