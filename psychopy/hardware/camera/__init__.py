...
from psychopy.tools.hardwaretools.cameratools import getCameraDescriptions, getCameras, CameraInfo, constants
...
class Camera:
    ...
    def __init__(self, device=0, mic=None, cameraLib=u'ffpyplayer',
        ...
        if ((api == constants.CAMERA_API_AVFOUNDATION and thisSystem != 'Darwin') or
                (api == constants.CAMERA_API_DIRECTSHOW and thisSystem != 'Windows')):
            raise RuntimeError(
    ...
    def _openWriter(self):
        ...
        self._tempVideoFileName = os.path.join(
            self._tempRootDir, constants.CAMERA_TEMP_FILE_VIDEO)
        self._tempAudioFileName = os.path.join(
            self._tempRootDir, constants.CAMERA_TEMP_FILE_AUDIO)
    ...
    def open(self):
        ...
        _camera = constants.CAMERA_NULL_VALUE
        _frameRate = constants.CAMERA_NULL_VALUE
        _cameraInfo = self._cameraInfo

        # setup commands for FFMPEG
        if _cameraInfo.cameraAPI == constants.CAMERA_API_DIRECTSHOW:  # windows
            ff_opts['f'] = 'dshow'
        ...
        elif _cameraInfo.cameraAPI == constants.CAMERA_API_AVFOUNDATION:  # darwin
            ...
            if math.isclose(constants.CAMERA_FRAMERATE_NTSC, _cameraInfo.frameRate):
                _frameRate = constants.CAMERA_FRAMERATE_NOMINAL_NTSC
            else:
