...
# this thing has a fake hole created by vertices - set line width to 2 to see the trick
thingVert = [
    (0,0),(0,.4),(.4,.4),(.4,0),(.1,0),(.1,.1),(.3,.1),(.3,.3),(.1,.3),(.1,0),(0,0),(.1,-.1),(.3,-.1),(.3,-.3),(.1,-.3),(.1,-.1)
]
thing = ShapeStim(
    win, 
    vertices=thingVert, 
...
# a list-of-lists will create a true hole, like a donut
donutVert = [
    [(-.2,-.2),(-.2,.2),(.2,.2),(.2,-.2)],
...
# lines are ok, use closeShape=False so it doesn't join up at the end
...
# shapes can be as complex as you like, even with very many vertices (like this coastline)
...
while not event.getKeys():
    ...
    lineA.draw()
    coast.draw()
    # shrink and rotate the star
    star7.ori += 1
    star7.size = star7.ori % 360 / 360
    # rotate the thing (slowly)
    thing.ori = star7.ori / 7
    # mess around with the selfx vertices
    selfxVert[0] = star7.size / 5
    selfxVert[3] = star7.size / 5 * (0, .9)
    selfx.vertices = selfxVert
    # draw instructions
    instr.draw()
    # flip window
    win.flip()
